...
class Board:
    ...
    def get_worker_moves(self, worker):
        legal_moves = self.get_legal_moves(self.curr_player)
        valid_squares = []

        for move in legal_moves:
            if move.worker == worker:
                new_row = move.get_new_coords()[0]
                new_col = move.get_new_coords()[1]
                new_square = self.get_square(new_row, new_col)
                valid_squares.append(new_square)
    ...
    def get_worker_builds(self, worker):
        legal_builds = self.get_legal_builds(self.curr_player, worker)
        valid_squares = []
    ...
    # def get_worker_builds(self, worker):
    #     legal_builds = self.get_legal_builds(self.curr_player)
    #     worker_builds = []

    #     for build in legal_builds:
    #         if build.worker == worker:
    #             worker_builds.append(build)

    #     return worker_builds
    ...
    def check_won(self, player):
        ...
        legal_opponent_moves = self.get_legal_moves(opponent)
        if len(legal_moves) == 0:
            messagebox.showinfo(title="YOU WON SANTORINI. yay :)", message="{} has won".format(opponent.color))
            # print("{} has won".format(opponent.color))
            self.playgame.window.quit()
            # return opponent
        if len(legal_opponent_moves) == 0:
            messagebox.showinfo(title="YOU WON SANTORINI. yay :)", message="{} has won".format(player.color))
            self.playgame.window.quit()
    ...
    def execute_move(self, move):
        ...
        self.check_won(self.curr_player)
